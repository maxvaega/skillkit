...
- Q: After scripts are detected during skill invocation, how are they made available to the agent for execution in LangChain? → A: Each detected script becomes a separate StructuredTool (e.g., skill "pdf-extractor" with 3 scripts generates 3 tools: "pdf-extractor__extract", "pdf-extractor__convert", "pdf-extractor__parse")
...
- Q: How should tool IDs be validated to ensure compatibility with LLM inference providers? → A: Validation regex ^[a-z0-9-]+__[a-z0-9_]+$ with max 60 chars total, enforced during tool registration with clear error message; absolutely no backward compatibility with old skill-name.tool_name format
- Q: Which specific files need to be updated for the tool ID format change? → A: Update core modules (langchain.py, manager.py) + all test files + README.md
- Q: What exception type should be raised when a generated tool ID fails validation (invalid format or exceeds 60 chars)? → A: New custom exception ToolIDValidationError (subclass of SkillkitError)
- Q: How should the system handle existing skills/deployments that may have been tested with the old skill-name.tool_name format? → A: N/A - this is new functionality, no existing deployments use script tools yet
- Q: Should migration/upgrade documentation be created to explain the tool ID format change? → A: Add documentation in examples/script_execution.py and README.md explaining the tool ID format (this is a new feature, not a migration)
...
- **FR-019**: Tool ID format change (from skill-name.tool_name to skill-name__tool_name) MUST be implemented across all core modules (src/skillkit/integrations/langchain.py, src/skillkit/core/manager.py), all test files, and README.md documentation
- **FR-020**: Tool ID format documentation MUST be added to README.md and examples/script_execution.py explaining the format requirements (lowercase skill name with hyphens/digits + double underscore + lowercase script name with underscores/digits, max 60 chars total, validated with regex ^[a-z0-9-]+__[a-z0-9_]+$) with rationale for LLM provider compatibility
...
- **ToolIDValidationError**: Exception raised when a generated tool ID fails validation (invalid format or exceeds 60 character limit); subclass of SkillkitError with descriptive error message including the invalid tool ID and format requirements
